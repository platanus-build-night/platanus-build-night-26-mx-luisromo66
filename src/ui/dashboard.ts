// dashboard.ts — Vista de estadísticas y gráficas en pestaña propia.
...
function renderWeekBars(heatmap: number[][]) {
  ...
  daily.forEach((mins, i) => {
    ...
    val.textContent = `${Math.round(mins)}m`;
...
function renderHeatmap(heatmap: number[][]) {
  ...
  heatmap.forEach((day, i) => {
    ...
    for (let h = 0; h < 24; h++) {
      ...
      cell.className = 'cell';
      const v = day[h] / max;
      cell.style.background = v > 0 ? `rgba(108,92,231,${0.2 + v * 0.8})` : '#1d2348';
